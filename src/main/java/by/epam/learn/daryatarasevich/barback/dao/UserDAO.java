...
public class UserDAO extends DAO<User> {
     ...
     * @param theUserID
     * @return theUser
     * @throws SQLException,ConnectionPoolException,NoSuchUserException
     */
    @Override
    ...
    public void registerUser(User theUser) {
        ...
        try {
            myConn = ConnectionPool.POOL.getConnection();
            String sql = "INSERT INTO barbackdb.users ( userNameEN, userNameRU, email, password, status, description ) values ( ?, ?, ?, ? , ?, ? )";
            myStmt = myConn.prepareStatement(sql);
            myStmt.setString(1, theUser.getName());
            myStmt.setString(2, theUser.getNameRU());
            myStmt.setString(3, theUser.getEmail());
    ...
    /**
     * To change user status defined by user ID to new status.
     *
     * @param userID
     * @param newStatus
